...
const EditTier = () => {
    ...
    const { updateTier, tier, isSuccess, fetchTiersById } = useTierStore()
    ...
    useEffect(() => {
        if (tier) {
            ...
            setValue('pointRangeFrom', tier?.pointRangeFrom)
            setValue('pointRangeTo', tier?.pointRangeTo)
            setUploadedImageId(tier.image.id)
            setUploadedImageUrl(tier.image.url)
        }
    }, [tier, setValue])
    ...
    const onSubmit = async (data) => {
        ...
        await updateTier(id, submittedData)
    }
