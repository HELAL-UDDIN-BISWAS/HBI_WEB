...
const EditDoctorPage = ({ id }) => {
    const { doctor, fetchSingleDoctor } = useDoctorStore();
    ...
    useEffect(() => {
        if (doctor) {
            ...
            setValue('doctorEmail', doctor?.email || '');
            setValue('number', doctor?.number || '');
            setValue('gender', doctor?.gender || '');
            setValue('education', doctor?.education || '');
            setValue('clinic', doctor?.clinic?.name || '');
            setValue('speciality', doctor?.speciality || '');
            setValue('subSpeciality', doctor?.subSpecialty || '');
            setValue('affiliatedHospital', doctor?.affiliateHospital || '');
            setValue('keyword', doctor?.keyword || '');
            setValue('professionalProfile', doctor?.professionalProfile || '');
        }
    }, [doctor, setValue]);
    ...
    const onSubmit = async (data) => {
        ...
        formData?.append('affiliatedHospital', data.affiliatedHospital);
        formData?.append('keyword', data.keyword);
        formData?.append('professionalProfile', data.professionalProfile);
                ...
                <TabPanel>
                    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-x-5 gap-y-4 mx-auto border-x-2 border-b-2 p-4 rounded-b-md rounded-se-md shadow-md bg-white">
                        ...
                        <div className="flex gap-3 col-span-2 mt-3">
                            <button type="submit" className="bg-[#3fc9c1] text-white py-2 px-4 rounded">Save Changes</button>
                            <button type="button" onClick={() => console.log('Cancel')} className="bg-[#dffffd] text-[#3FC9C1] border-1 border-[#3FC9C1] py-2 px-4 rounded">Cancel</button>
                        </div>
                    </form>
                </TabPanel>
            </Tabs>
